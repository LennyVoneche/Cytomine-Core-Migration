...
//    def grailsApplication
    ...
    def check() {
        try {
            ...
            DependencyController.log.error e
        }
    ...
    public def checkDependence() {
        ...
        grailsApplication.getDomainClasses().each { domain ->
            if(!domainToSkip.contains(domain.name)) {
                def columnDep = getDependencyColumn(domain,["cytomine.core."],["cytomine.core."])
                allErrors.addAll(checkServiceMethod(domain,columnDep,fixDomainName))

                def columnDepHasMany = getDependencyColumn(domain,["java.util.Set","java.util.SortedSet","java.util.List"],["cytomine.core."])
                allErrors.addAll(checkServiceMethodInverse(domain,columnDepHasMany,fixDomainName))
            }
        }

        DependencyController.log.error "There are ${allErrors.size()} dependency conflict!"

        allErrors.sort().each {
            DependencyController.log.error it
        }
